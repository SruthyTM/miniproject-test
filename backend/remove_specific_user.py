...
def remove_specific_user():
    db = SessionLocal()
    try:
        email_to_remove = "sruthyymuraleedharn@gmail.com"  # Note: typo in email
        ...
        if not user:
            ...
            # Let's also check for similar emails
            all_users = db.query(User).filter(User.email.like("%sruthy%")).all()
            if all_users:
